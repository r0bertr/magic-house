...
- 太阳位置按照椭圆轨迹变化：太阳模型和相应的变化（position,direction,ambient,diffuse,specular）
- 光照方向变化
- 导入了一个夜晚天空盒，天空盒使用两个进行平滑切换，通过apha进行控制
- 这些apha,光照分量等等都是通过时间t的三角函数形成的。这些三角函数，根据这些不同量的平滑变化区间而设置，这些变化都是通过多次尝试模拟以及经验尝试筛选出的最好的昼夜变化的效果。
